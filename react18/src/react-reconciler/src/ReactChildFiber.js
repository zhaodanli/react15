...
/**
 * 这个文件实现了 React Fiber 架构中子节点调和（Reconciliation）的核心算法，
 * 主要负责新旧子节点的对比、复用、插入、删除和移动，生成新的 Fiber 子链表（成新的 Fiber 节点树），
    * 新的子节点（newChild）与当前 Fiber 树中的子节点（currentFirstChild）
 * 并为后续的 commit 阶段提供精确的副作用标记
 * /
...
function createChildReconciler(shouldTrackSideEffects) {
    ...
    /** 副作用标记逻辑 */
    // reconcileChildFibers[主要函数调用] -> placeSingleChild
    function placeSingleChild(newFiber) {
        // 说明要添加副作用 shouldTrackSideEffects true：表示当前是“更新阶段”，需要跟踪副作用
    ...
    // 复用 fiber
    // reconcileChildFibers[主要函数调用] -> reconcileChildrenArray -> updateSlot -> updateElement -> useFiber
    // reconcileChildFibers[主要函数调用] -> reconcileChildrenArray -> updateFromMap -> updateTextNode -> updateElement -> useFiber
    // reconcileChildFibers[主要函数调用] -> reconcileChildrenArray -> updateFromMap -> updateTextNode -> useFiber
    // reconcileChildFibers[主要函数调用] -> reconcileSingleElement -> useFiber
    ...
    // 删除所有剩余的子节点。
    // reconcileSingleElement -> deleteRemainingChildren 
    // reconcileChildrenArray -> deleteRemainingChildren
    ...
    // reconcileSingleElement -> deleteChild
    // reconcileSingleElement -> deleteRemainingChildren -> deleteChild
    // reconcileChildrenArray -> deleteChild
    // reconcileChildrenArray -> deleteRemainingChildren -> deleteChild
    ...
    * 2.2 如果key不同，删除当前fiber, 继续查找下一个 fiber节点A(key="a") -> B(key="b") -> C(key="c")
    ...
    /** 副作用标记逻辑
     * 为新创建的 Fiber 节点设置索引（index）和副作用标记（Placement）。 用于后续 DOM 插入或移动。
     */
    ...
    // 映射剩下的老节点
    // reconcileChildrenArray -> mapRemainingChildren
    ...
    // reconcileChildrenArray -> updateFromMap
    ...
    // reconcileChildrenArray -> updateFromMap -> updateTextNode
    ...
    // reconcileChildrenArray -> updateSlot -> updateElement
    // reconcileChildrenArray -> updateFromMap -> updateTextNode -> updateElement
